	...
	<div class="row">
		<div class="col-md-6">
			<div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
				<label >Title</label>
				<input type="text" name="title" id="" class="form-control" value="{{ old('title') }}" placeholder="Enter CMS page title">
				@if($errors->has('title'))
		...
		<div class="form-group{{ $errors->has('parent_id') ? ' has-error' : '' }}">
			...
			<div class="d-flex">
				<label >
					<input type="radio" name="main_or_sub" class="main_or_sub" value="1"  @if (old('main_or_sub')==1)
                        checked
                    @endif> Main
				</label>
				<label class="mx-3">
					<input type="radio" name="main_or_sub" class="main_or_sub" value="0" @if (old('main_or_sub')===0)
                        checked
                    @endif> Sub
				</label>
			</div>
		...
		<div class="col-md-12">
			...
			<div  id="maincms" class="  @if (old('main_or_sub')===0)
                d-block
                @else 
                d-none
            @endif">	
				<div class="form-group">
                    ...
                    <select name="parent_id" id="" class="form-control">
                        ...
                        <option value="{{ $item->id }}" @if ($item->id==old('parent_id'))
                            selected
		...
		<div class="col-md-6">
				...
				<div class="form-group{{ $errors->has('position') ? ' has-error' : '' }}">
					...
					<label > Position:</label>
						<input type="number" name="position" id="" placeholder="Enter Postion value" required class="form-control" value="{{ old('position') }}"> 
            ...
            <div class="form-group">    
                <label class="ckbox ckbox-success">
                   <p>Hide in Header</p>
					...
					<input type="checkbox" name="hide_header" id="" value="1"  class="form-control" @if (old('hide_header')==1)
                        checked
                    @endif> 
                </label>
